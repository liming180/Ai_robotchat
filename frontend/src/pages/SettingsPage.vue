...
const companionStore = useCompanionStore() as import('@/types/companion-store').CompanionsStore as any
...
const handleAvatarError = (e: Event) => {
  ...
  if (!img) return
  const name = form.value.name || companion.value?.name || 'AI'
  const label = name.trim().slice(-1) || 'AI'
  const seed = `${route.params.id || 'companion'}-${label}`
